...
def create_excel_manual_from_json_file(json_file_path, output_excel_path,agb_excel_path):
    ...
    for item in json_data:
        ...
        for ingredient in item['Ingredient']:
            ...
            closest_match = get_closest_match(item['Name'], ingredient['Nom'], choices, match_cache)
...
def create_excel_iscala_from_json_file(json_file_path, output_excel_path,agb_excel_path):
    ...
    for item in json_data:
        ...
        for ingredient in item['ingredients']:
            ...
            closest_match = get_closest_match(item['name'], ingredient['name'], choices, match_cache)
            agb_code = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'Code\nAGB'].values[0]
            cat_name = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'WiseFins EN Category'].values[0]
            subcat_name = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'WiseFins EN Subcategory'].values[0]
...
def create_excel_checkscm_ingre_from_json_file(json_file_path, output_excel_path, agb_excel_path):
    agb_df = pd.read_excel(agb_excel_path, engine='openpyxl')
    ...
    # Preparing data for batch fuzzy matching
    rows = []
    match_cache = {}
    ...
    for item in json_data:
        ...
        # Get the closest match for ingredient name from the AGB DataFrame
        closest_match = get_closest_match(item['Group'], item['Product Description'], choices, match_cache)
        agb_code = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'Code\nAGB'].values[0]
        cat_name = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'WiseFins EN Category'].values[0]
        subcat_name = agb_df.loc[agb_df['LCI Name'] == closest_match[0], 'WiseFins EN Subcategory'].values[0]
        row = {
...
def merge_all_excel_files_in_directory(directory_path, output_file):
    ...
    excel_files = directory.rglob('*.xlsx')
...
def main():
    ...
    # Dossier Data 
    # A modifier selon votre dispostion
    ...
    for json_path, excel_path in zip(json_file_iscala_paths, excel_output_iscala_paths):
        create_excel_iscala_from_json_file(json_path, excel_path,agb_file)
    ...
    for json_path, excel_path in zip(input_checkscm_ingre_json_files, excel_checkscm_ingre_files):
        create_excel_checkscm_ingre_from_json_file(json_path, excel_path,agb_file)
    ...
    for json_path, excel_path in zip(input_manual_json_files, excel_output_manual_files):
        create_excel_manual_from_json_file(json_path, excel_path,agb_file)
    ...
    merge_all_excel_files_in_directory(directory_path, "data_To_Load.xlsx" )
